...
func TestH3CCPUNative(t *testing.T) {
	for _, version := range []string{"v1", "v2c"} {
		SrvTest(t, "../data_store/etc/tpt_models.xml", func(client *ds.Client, sampling_url string, definitions *types.TableDefinitions) {
			...
			res := nativeGet(t, sampling_url, "121.0.0.4", "cpu", map[string]string{"snmp.version": version, "snmp.read_community": "public"})
			if res.HasError() {
				t.Error(res.Error())
				return
			}
