...
func TestIcmpResultBuffer(t *testing.T) {
	cb := NewIcmpBuffer(make([]IcmpResult, 10))
	check := func(cb *icmpBuffer, c int64, isNoCommit bool) {
		if c < 10 {
			all := cb.All()
			if isNoCommit {
				if cb.Size() != int(c) {
			...
			if int64(c-9) != cb.First().SampledAt {
